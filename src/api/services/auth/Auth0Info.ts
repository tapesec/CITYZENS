...
class Auth0Info {
    ...
    public async updateMetadata(id: string, accessToken: string, data: any){
        return new Promise<any>(async (resolve, reject) => {
            this.auth0.updateUserMetadataById(id, data)
                .then((v) => {
    ...
    public async getUserInfo(accessToken: string){
        ...
        return new Promise<UserInfoAuth0>(async (resolve, reject) => {
            await this.auth0.getUserInfo(accessToken).then(body => {
    ...
    public login(username: string, password: string){
        return new Promise(async (resolve, reject) => {
            await this.auth0.login(username, password)
                .then(r => resolve(r))
                .catch(err => {
