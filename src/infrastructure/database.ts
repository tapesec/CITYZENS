/**
 * Ce script crée les schemas de base de données dans
 * tous les environnements dans lesquels ils sont joués.
 * Des données de tests sont insérés uniquement en local et test si ils n'existent pas déjà.
 * Pour la table citys les données sont insérés dans tous les env si ils n'existent pas.
 */
...
export const initDB = async () => {
    ...
    try {
        ...
        let response = await pg.query(
            `SELECT EXISTS(SELECT user_id FROM cityzens WHERE user_id=$1) as exist`,
            [CityzenSample.ELODIE.id.toString()],
        );

        if (
            response.rows[0].exist === false &&
            ['test', 'local'].indexOf(config.server.env) !== -1
        ) {
            insertCityzens();
        }
        ...
        response = await pg.query(`SELECT EXISTS(SELECT id FROM messages WHERE id=$1) as exist`, [
            MessageSample.MARTIGNAS_CHURCH_MESSAGE.id.toString(),
        ]);
        ...
        response = await pg.query(`SELECT EXISTS(SELECT id FROM hotspots WHERE id=$1) as exist`, [
            MediaHotspotsSample.CHURCH.id.toString(),
        ]);
        ...
        response = await pg.query(
            `SELECT EXISTS(SELECT insee FROM citys WHERE insee=$1) as exist`,
            [CitySample.MARTIGNAS.insee],
        );

        if (response.rows[0].exist === false) {
            insertCitys();
        }
    } catch (error) {
...
const insertCityzens = async () => {
    ...
    for (const entry of cityzens) {
        ...
        const favorites =
            cityzen.favoritesHotspots !== undefined ? Array.from(cityzen.favoritesHotspots) : [];
